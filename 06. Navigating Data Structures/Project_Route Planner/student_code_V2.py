...
def whattoexpandnext(path):
    minval=0
    expnd=0 # expnd returns zero - this means, in the path list there are no end nodes on frontier that are not in explored or are not goals or have some connecting nodes.
    
    #Find minimum path cost node in the path list that has not been explored or is not the goal
    for p in path:
        if path[p][-2] not in explored:
            if path[p][-2] != goal:
                minval=path[p][-1]
                expnd=p
    # Use this minimum value as base to find which path to expand only if that node has connecting nodes.
    for p in path:    
        if path[p][-1]<minval:
            if path[p][-2] not in explored:
                if path[p][-2] != goal:
...
def expandedpath(path,roads,ex):
    ...
    curpath=[]
    exoriginal=0
    exoriginal=ex # I did this dual assignment as python was just assigning the reference to ex with a straight assignment.
