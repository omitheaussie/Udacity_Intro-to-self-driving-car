...
*/
#ifndef LOCALIZER_CPP
#define LOCALIZER_CPP
	...
	float p_miss) 
{
	...
	vector< vector <float> > newGrid;
	int i, j;
	vector<char> row_color;
	vector<float> row_beliefs;
	int hit;

	for (i = 0; i < grid.size(); i++) {
		row_color = grid[i];
		for (j = 0; j < grid[i].size(); j++) {
			hit = (row_color[j] == color);
			row_beliefs.push_back(beliefs[i][j] * p_hit* hit + beliefs[i][j] * (1 - hit)*p_miss);
		}
		newGrid.push_back(row_beliefs);
		row_color.clear();
		row_beliefs.clear();
	}
